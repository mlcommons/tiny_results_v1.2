...
float input_float[AI_NETWORK_IN_1_SIZE];
...
{
    int8_t in_zero_point = *((int8_t *)ai_input->meta_info->intq_info->info->zeropoint);
    float in_scale =  *(ai_input->meta_info->intq_info->info->scale);
    
    size_t bytes = ee_get_buffer((uint8_t *)(input_float),AI_NETWORK_IN_1_SIZE * sizeof(float));
    if (bytes / sizeof(float) != AI_NETWORK_IN_1_SIZE) {
        th_printf("Input db has %d elemented, expected %d\n", bytes / sizeof(float),AI_NETWORK_IN_1_SIZE);
        return;
    }
    for (int i = 0; i < AI_NETWORK_IN_1_SIZE; i++)
    {
        aiInData_int[i] = QuantizeFloatToInt8(input_float[i], in_scale, in_zero_point);
    }
...
void th_results(void)
{
    ...
    const int n_samples = ai_output->size;
    float diff = 0.0;
    float diffsum = 0;
    float res = 0.0;
    char char_res[10];
    ...
    */
    ...
    {
        float converted = DequantizeInt8ToFloat(aiOutData[i], out_scale, out_zero_point);
        
        diff = converted - input_float[i];
        diffsum += diff * diff;
    }
    res = diffsum / n_samples;
    snprintf(char_res, 10, "%0.3f",res);
    th_printf("m-results-[");
    th_printf(char_res);
    th_printf("]\r\n");
