...
void set_input(const float *custom_input) {
	memcpy(in_out.input, custom_input, 4 * 27648);
}
...
float dequantize(const int8_t value) {
	return nn_Identity_int8_dequantize_int8_to_float(value);
}
*/
...
void th_load_tensor() {
    ...
    float zp = -128.0;
    float scale = 0.003921568859368563F;
    for (size_t i = 0; i < bytes; i++) {
    	input_asfloat[i] = ((float) input[i] - zp) * scale;
    	if (input_asfloat[i] > 1.0) {
    		input_asfloat[i] = 1.0;
    	}
    }
