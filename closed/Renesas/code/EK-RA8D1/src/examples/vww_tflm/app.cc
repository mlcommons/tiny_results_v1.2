...
constexpr uint32_t model_input_size = (96 * 96 * 3);
constexpr uint32_t tensor_arena_size = (100 * 1024);
constexpr uint32_t max_result_buffer_size = 128;
constexpr uint32_t category_size = 2;

// notice sign
static int8_t input_buffer[model_input_size];
...
void app_load_tensor(size_t (*get_buffer_func)(uint8_t* input, size_t size)){
    size_t bytes = get_buffer_func(reinterpret_cast<uint8_t *>(input_buffer), model_input_size * sizeof(int8_t));
