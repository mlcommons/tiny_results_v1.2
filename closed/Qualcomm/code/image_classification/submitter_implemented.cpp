...
float calculate_result(struct eai_sample_context *context)
{
    ...
    for (int i = 0; i < context->tensor_count[1]; i++)
    {
        ...
        if (nresults != tensor_o->tensor_size / sizeof(int8_t))
        {
            th_printf("output data type is int32, output data dimension: %lu != 10,", tensor_o->tensor_size / sizeof(int32_t));
        }
        for (unsigned int j = 0; j < tensor_o->tensor_size / sizeof(int8_t); j++)
        {
            // int8 to float
...
int Uint8ToInt8(uint8_t *address, uint32_t tensor_size)
{
    ...
    {
        // int8_t tmp = *p / 2;
        // tmp = tmp * 2.0154189165900736/2.086614;
        int8_t tmp = (float)*p / 2.094488189;
