...
// #include "mbed.h"
float g_result[16] = {0};
//extern "C" void write_ai_data(void);
...
extern "C" int quant_resnet_uint8(void);
void th_infer() 
{
	quant_resnet_uint8();
}
...
extern "C" unsigned long get_current_time(uint8_t timer_num);
extern "C" uint64_t get_current_clock(void);
void th_timestamp(void) {
  unsigned long microSeconds = 0ul;
  /* USER CODE 2 BEGIN */
  // microSeconds = us_ticker_read();
  //microSeconds = get_current_time(0);
  microSeconds = get_current_clock();
  /* USER CODE 2 END */
